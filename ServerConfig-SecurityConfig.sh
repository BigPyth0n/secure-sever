...
# =============================================
# تابع اتصال به CrowdSec Console با کنترل خطا
# =============================================
connect_to_console() {
    ...
    if [ $status -eq 0 ]; then
        SERVICE_STATUS["crowdsec_console"]="✅ Active (Email: $CONSOLE_EMAIL)"
        send_telegram "🎉 *CrowdSec Console Connection Successful*  
        \\- Email: \`$CONSOLE_EMAIL\`  
        \\- Dashboard: [console.crowdsec.net](https://console.crowdsec.net)"
        return 0
    else
        SERVICE_STATUS["crowdsec_console"]="❌ Failed (Check logs)"
