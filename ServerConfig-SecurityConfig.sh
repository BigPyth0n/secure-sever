...
install_crowdsec() {
    ...
    # آپدیت CrowdSec Hub
    cscli hub update || echo "⚠️ خطا در آپدیت Hub، ادامه می‌دهیم..."
    
    # مجموعه‌ها
    local collections=(
        "crowdsecurity/sshd"           # برای SSH و SFTP
        "crowdsecurity/nginx"          # برای Nginx Proxy Manager
        "crowdsecurity/mysql"          # برای MySQL
        "crowdsecurity/linux"          # امنیت کلی
        "crowdsecurity/http-cve"       # حملات HTTP
        "crowdsecurity/base-http-scenarios"  # پایه HTTP
    )
    ...
    # نصب پارسرها و سناریوها
    cscli parsers install crowdsecurity/whitelists crowdsecurity/geoip-enrich || echo "⚠️ خطا در نصب پارسرها"
    cscli scenarios install crowdsecurity/http-probing crowdsecurity/ssh-bf || echo "⚠️ خطا در نصب سناریوها"
    
    # اضافه کردن لاگ‌های سرویس‌ها (برای نظارت کامل)
...
---
filenames:
  - /var/log/mysql.log  # اگه MySQL لاگ داره
labels:
  type: mysql
---
filenames:
  - /var/log/streamlit.log  # باید Streamlit رو تنظیم کنی که لاگ بده
labels:
  type: syslog
---
filenames:
  - /home/$NEW_USER/.local/share/code-server/logs/*  # لاگ‌های Code-Server
labels:
  type: syslog
---
filenames:
  - /var/log/portainer.log  # باید Portainer رو تنظیم کنی که لاگ بده
labels:
  type: syslog
---
filenames:
  - /var/log/netdata.log  # باید Netdata رو تنظیم کنی که لاگ بده
labels:
  type: syslog
EOL
