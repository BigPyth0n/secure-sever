...
# اضافه کردن مخزن با دسترسی root و بررسی
...
if docker ps >/dev/null 2>&1; then
    ...
    echo "Y" | sudo cscli dashboard setup --listen 0.0.0.0:$CROWDSEC_DASHBOARD_PORT --force
...
📌 **نکات مهم:**  
   - برای Portainer پس از ورود، رمز عبور را تغییر دهید.  
   - برای Code-Server از رمز \`$CODE_SERVER_PASSWORD\` استفاده کنید (توصیه: تغییر دهید).  
   - برای CrowdSec داشبورد: به http://$SERVER_IP:$CROWDSEC_DASHBOARD_PORT بروید (نام کاربری: admin@crowdsec.net، رمز اولیه: crowdsec).  
