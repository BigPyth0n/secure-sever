...
# کلید عمومی جدیدت رو مستقیم اینجا می‌ذارم
PUBLIC_KEY="ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIInEoyFHDWd3D/WhQSQfkzH/p3P5lD3/OoVslSjVzqwL bigpyth0n@TradePC"
...
# 🛠️ 3. ایجاد کاربر جدید و تنظیم کلید عمومی
...
if ! id "$NEW_USER" &>/dev/null; then
    ...
    echo "$NEW_USER ALL=(ALL) NOPASSWD: ALL" | tee /etc/sudoers.d/"$NEW_USER" || { echo "❌ Failed to set sudoers"; exit 1; }
    chmod 440 /etc/sudoers.d/"$NEW_USER"
fi

# اطمینان از وجود دایرکتوری خانگی
if [[ ! -d "/home/$NEW_USER" ]]; then
    mkdir -p "/home/$NEW_USER" || { echo "❌ Failed to create home directory"; exit 1; }
    chown "$NEW_USER":"$NEW_USER" "/home/$NEW_USER" || { echo "❌ Failed to set ownership for home directory"; exit 1; }
fi

# ایجاد و تنظیم دایرکتوری .ssh با کلید عمومی جدید
echo "🔑 Setting up SSH directory and public key for $NEW_USER..."
mkdir -p "/home/$NEW_USER/.ssh" || { echo "❌ Failed to create .ssh directory"; exit 1; }
echo "$PUBLIC_KEY" > "/home/$NEW_USER/.ssh/authorized_keys" || { echo "❌ Failed to write public key"; exit 1; }
...
docker run -d --name portainer -p "$PORTAINER_PORT:9000" -v /var/run/docker.sock:/var/run/docker.sock -v portainer_data:/data --restart unless-stopped portainer/portainer-ce:latest || { echo "❌ Failed to run Portainer"; exit 1; }

# 🛠️ 5. نصب نسخه‌های مختلف پایتون
echo "🐍 Installing Python versions..."
apt install -y software-properties-common build-essential libssl-dev zlib1g-dev libbz2-dev libreadline-dev libsqlite3-dev wget curl llvm libncurses5-dev libncursesw5-dev xz-utils tk-dev libffi-dev liblzma-dev python3-openssl || { echo "❌ Failed to install Python prerequisites"; exit 1; }
...
docker run -d --name nginx-proxy-manager -p 80:80 -p 81:81 -p 443:443 -v /opt/nginx-proxy-manager/data:/data -v /opt/nginx-proxy-manager/letsencrypt:/etc/letsencrypt --restart unless-stopped jc21/nginx-proxy-manager:latest || { echo "❌ Failed to run Nginx Proxy Manager"; exit 1; }
...
EOL
cscli notifications add /etc/crowdsec/notifications/http.yaml || { echo "❌ Failed to add notification"; exit 1; }
...
apt install -y wget curl net-tools iperf3 htop glances tmux rsync vim nano unzip zip build-essential git lftp clamav clamav-daemon rkhunter lynis auditd tcpdump nmap netdata || { echo "❌ Failed to install tools"; exit 1; }
...
chown secftpuser:secftpuser "/home/$NEW_USER" || { echo "❌ Failed to set ownership for FTP"; exit 1; }
chmod 750 "/home/$NEW_USER" || { echo "❌ Failed to set permissions for FTP"; exit 1; }
...
1. FTP (vsftpd)
   آدرس: ftp://$SERVER_IP:2121
   نام کاربری: secftpuser
   رمز: YumJdc\$Qvs3mZ^*dFJxa
   توضیحات: دسترسی به /home/bigpython
2. Code-Server
   آدرس: http://$SERVER_IP:$CODE_SERVER_PORT
   رمز: $CODE_SERVER_PASSWORD
3. CrowdSec Dashboard
   آدرس: http://$SERVER_IP:$CROWDSEC_DASHBOARD_PORT
   نام کاربری: crowdsec@crowdsec.net
   رمز: $CROWDSEC_PASSWORD
4. Netdata
   آدرس: http://$SERVER_IP:$NETDATA_PORT
5. Nginx Proxy Manager
   آدرس: http://$SERVER_IP:81
   نام کاربری: پیش‌فرض
   رمز: پیش‌فرض (بعد از ورود تغییر دهید)
6. Portainer
   آدرس: http://$SERVER_IP:$PORTAINER_PORT
   توضیحات: اولین ورود رمز تنظیم کنید
